...
/**
 * Detect HTTPS requests even when WordPress sits behind a proxy/CDN.
 *
 * @return bool
 */
function udsp31_is_https_request() {
	if ( is_ssl() ) {
		return true;
	}
	...
	return '443' === $server_port;
}

/**
 * Return the current request host, including proxy headers when present.
 *
 * @return string
 */
function udsp31_get_request_host() {
	$forwarded_host = isset( $_SERVER['HTTP_X_FORWARDED_HOST'] ) ? trim( (string) $_SERVER['HTTP_X_FORWARDED_HOST'] ) : '';
	if ( $forwarded_host ) {
		$hosts = array_map( 'trim', explode( ',', $forwarded_host ) );

		if ( ! empty( $hosts[0] ) ) {
			return preg_replace( '/:\d+$/', '', $hosts[0] );
		}
	}

	$http_host = isset( $_SERVER['HTTP_HOST'] ) ? trim( (string) $_SERVER['HTTP_HOST'] ) : '';
	if ( $http_host ) {
		return preg_replace( '/:\d+$/', '', $http_host );
	}
...
/**
 * Return the current request scheme.
 *
 * @return string
 */
function udsp31_get_request_scheme() {
	return udsp31_is_https_request() ? 'https' : 'http';
}

/**
 * Check whether a host belongs to the local site stack.
 *
 * @param string $host Host name to inspect.
 * @return bool
 */
function udsp31_is_local_host( $host ) {
	...
	return in_array(
		$host,
		array(
			'eghyxsy.cluster100.hosting.ovh.net',
			'www.udsp31.fr',
			'udsp31.fr',
		),
		true
...
/**
 * Rewrite local site URLs to the current scheme and host.
 *
 * @param string $url URL to normalize.
 * @return string
 */
function udsp31_normalize_local_url( $url ) {
	if ( ! $url ) {
		return $url;
	}
	...
	if ( ! udsp31_is_local_host( $parsed['host'] ) ) {
		return $url;
	}

	$request_host = udsp31_get_request_host();
	$target_host  = $request_host ? $request_host : $parsed['host'];
	$target_url   = udsp31_get_request_scheme() . '://' . $target_host;
...
/**
 * Normalize local asset URLs for the current scheme and host.
 *
 * @param string $url Asset URL.
 * @return string
 */
function udsp31_force_https_url( $url ) {
	if ( ! $url ) {
		return $url;
	}
	...
	if ( empty( $parsed['host'] ) || empty( $parsed['path'] ) ) {
		return udsp31_is_https_request() ? set_url_scheme( $url, 'https' ) : $url;
	}

	$is_local_asset = false;

	foreach ( array( '/wp-content/', '/wp-includes/', '/wp-admin/' ) as $prefix ) {
		if ( 0 === strpos( $parsed['path'], $prefix ) ) {
			$is_local_asset = true;
			break;
		}
	}
	...
	$scheme       = udsp31_is_https_request() ? 'https' : ( ! empty( $parsed['scheme'] ) ? $parsed['scheme'] : 'http' );
	$request_host = udsp31_get_request_host();
	$host         = $request_host ? $request_host : $parsed['host'];
	$rebuilt_url  = $scheme . '://' . $host . $parsed['path'];
...
/**
 * Keep enqueued style and script URLs on HTTPS for proxied requests.
 *
 * @param string $src Asset source URL.
 * @return string
 */
function udsp31_force_https_asset_src( $src ) {
	return udsp31_force_https_url( $src );
}
add_filter( 'style_loader_src', 'udsp31_force_https_asset_src', 20 );
add_filter( 'script_loader_src', 'udsp31_force_https_asset_src', 20 );

/**
 * Normalize local site URLs generated by WordPress to the current host.
 *
 * @param string $url URL generated by WordPress.
 * @return string
 */
function udsp31_normalize_generated_local_url( $url ) {
	return udsp31_normalize_local_url( $url );
}
add_filter( 'option_home', 'udsp31_normalize_generated_local_url', 20 );
add_filter( 'option_siteurl', 'udsp31_normalize_generated_local_url', 20 );
add_filter( 'home_url', 'udsp31_normalize_generated_local_url', 20 );
add_filter( 'site_url', 'udsp31_normalize_generated_local_url', 20 );
add_filter( 'content_url', 'udsp31_normalize_generated_local_url', 20 );
add_filter( 'theme_mod_udsp31_hero_tertiary_url', 'udsp31_normalize_generated_local_url', 20 );
add_filter( 'wp_get_attachment_url', 'udsp31_normalize_generated_local_url', 20 );

/**
 * Redirect retired theme pages away from public navigation.
 *
 * @return void
 */
function udsp31_redirect_retired_pages() {
	if ( is_admin() || wp_doing_ajax() ) {
		return;
	}

	if ( is_page( array( 'annuaire', 'agenda', 'politique-de-confidentialite' ) ) ) {
		wp_safe_redirect( home_url( '/' ), 301 );
		exit;
	}
}
add_action( 'template_redirect', 'udsp31_redirect_retired_pages' );
...
function udsp31_scripts() {
	...
	wp_enqueue_style( 'udsp31-style', udsp31_force_https_url( get_stylesheet_uri() ), array(), $style_ver );
	wp_enqueue_script(
		'udsp31-script',
		udsp31_force_https_url( get_template_directory_uri() . '/assets/js/theme.js' ),
		array(),
		$script_ver,
		true
	);
...
function udsp31_asset_url( $path ) {
	return udsp31_force_https_url( trailingslashit( get_template_directory_uri() ) . ltrim( $path, '/' ) );
}
...
function udsp31_get_bundled_logos() {
	...
	foreach ( $patterns as $pattern ) {
		...
		foreach ( $matches as $path ) {
			...
			$file_name = wp_basename( $path );
			$variant   = udsp31_get_logo_variant_key( $file_name );
			$url       = udsp31_asset_url( 'assets/images/' . rawurlencode( $file_name ) );
	...
	if ( empty( $logos['default'] ) && file_exists( get_template_directory() . '/assets/images/Logo_UDSP31_blue.png' ) ) {
		$path             = get_template_directory() . '/assets/images/Logo_UDSP31_blue.png';
		$logos['default'] = udsp31_build_logo_data( $path, udsp31_asset_url( 'assets/images/Logo_UDSP31_blue.png' ) );
...
/**
 * Return the news archive URL used by the theme.
 *
 * @return string
 */
function udsp31_get_news_archive_url() {
	$news_page = get_page_by_path( 'actualites' );
...
function udsp31_customize_register( $wp_customize ) {
	...
	$settings = array(
		...
		'udsp31_hero_primary_url'  => array(
			...
			'default'  => 'https://unions-pompiers.fr/Views/inscription_formation.php?departement=31',
		...
		'udsp31_hero_secondary_url' => array(
			...
			'default'  => 'https://unions-pompiers.fr/Views/demande_dps.php?departement=31',
			'sanitize' => 'esc_url_raw',
			'type'     => 'url',
		),
		...
		'udsp31_hero_tertiary_url' => array(
			...
			'default'  => udsp31_get_recruitment_url( 'devenir-jeune-sapeur-pompier' ),
			'sanitize' => 'esc_url_raw',
			'type'     => 'url',
		),
